...
enum QuizLeaderboardFactory {
    ...
    static func makeScene(withScore score: Int, questionCount: Int) -> QuizLeaderboardViewController {
        ...
        let presenter = QuizLeaderboardPresenter(view: vc, score: score, questionCount: questionCount)
        let worker = QuizLeaderboardWorker()
        let interactor = QuizLeaderboardInteractor(presenter: presenter, worker: worker)
